...
static struct {
    ...
    vec3s pos[MAX_PARTICLES];
    vec3s vel[MAX_PARTICLES];
} state;
...
static void init(void) {
    ...
    const float vertices[] = {
        // positions            colors
        0.0f, -r,   0.0f, 1.0f, 0.0f, 0.0f, 1.0f, r,    0.0f, r,    0.0f,
        1.0f, 0.0f, 1.0f, r,    0.0f, -r,   0.0f, 0.0f, 1.0f, 1.0f, -r,
        0.0f, -r,   1.0f, 1.0f, 0.0f, 1.0f, -r,   0.0f, r,    0.0f, 1.0f,
        1.0f, 1.0f, 0.0f, r,    0.0f, 1.0f, 0.0f, 1.0f, 1.0f};
    state.bind.vertex_buffers[0] = sg_make_buffer(&(sg_buffer_desc){
        .data = SG_RANGE(vertices), .label = "geometry-vertices"});

    // index buffer for static geometry
    const uint16_t indices[] = {0, 1, 2, 0, 2, 3, 0, 3, 4, 0, 4, 1,
    ...
    // empty, dynamic instance-data vertex buffer, goes into vertex-buffer-slot
    // 1
    state.bind.vertex_buffers[1] =
        sg_make_buffer(&(sg_buffer_desc){.size = MAX_PARTICLES * sizeof(vec3s),
        ...
        sg_make_pipeline(&(sg_pipeline_desc){
            .layout =
                {// vertex buffer at slot 1 must step per instance
                 ...
                 .attrs =
                     {[ATTR_instancing_pos] = {.format = SG_VERTEXFORMAT_FLOAT3,
                                               .buffer_index = 0},
                      [ATTR_instancing_color0] = {.format =
                                                      SG_VERTEXFORMAT_FLOAT4,
                                                  .buffer_index = 0},
                      [ATTR_instancing_inst_pos] = {.format =
...
static void frame(void) {
    ...
    for (int i = 0; i < NUM_PARTICLES_EMITTED_PER_FRAME; i++) {
        if (state.cur_num_particles < MAX_PARTICLES) {
            ...
            state.vel[state.cur_num_particles] = (vec3s){
                .x = ((float)(xorshift32() & 0x7FFF) / 0x7FFF) - 0.5f,
                .y = ((float)(xorshift32() & 0x7FFF) / 0x7FFF) * 0.5f + 2.0f,
                .z = ((float)(xorshift32() & 0x7FFF) / 0x7FFF) - 0.5f};
    ...
    sg_update_buffer(
        state.bind.vertex_buffers[1],
        &(sg_range){.ptr = state.pos,
                    .size = (size_t)state.cur_num_particles * sizeof(vec3s)});

    // model-view-projection matrix
    const mat4s proj = glms_perspective(
        glm_rad(60.0f), sapp_widthf() / sapp_heightf(), 0.01f, 50.0f);
    const mat4s view = glms_lookat((vec3s){.x = 0.0f, .y = 1.5f, .z = 8.0f},
                                   (vec3s){.x = 0.0f, .y = 0.0f, .z = 0.0f},
                                   (vec3s){.x = 0.0f, .y = 1.0f, .z = 0.0f});
    const mat4s view_proj = glms_mat4_mul(proj, view);
    state.ry += 60.0f * frame_time;
    vs_params_t vs_params;
    memcpy(&vs_params.mvp,
            glms_mat4_mul(view_proj, glms_rotate_make(glm_rad(state.ry),
            (vec3s){.x = 0.0f, .y = 1.0f, .z = 0.0f})).raw,
            sizeof(mat4s));
